...
namespace ffx::framework::concurrency {
  ...
  public:
    using concurrent_lanes_pool_t = std::vector<std::unique_ptr<ConcurrentLane<TQueue, TLaneCapacity>>>;
    ...
    ConcurrentLane<TQueue, TLaneCapacity>& concurrent_lane(const std::size_t batch_id) noexcept {
      // affine mapping
