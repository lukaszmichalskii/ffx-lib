...
namespace ffx::soa {
  ...
  public:
    using Product = T;
    using Buffer = ffx::device_buffer<TDev, Product>;
    using ConstBuffer = ffx::const_device_buffer<TDev, Product>;

    PortableDeviceObject() = delete;

    PortableDeviceObject(TDev const& device)
        // allocate global device memory
        : buffer_{ffx::make_device_buffer<Product>(device)} {
    ...
    PortableDeviceObject(TQueue const& queue)
        ...
        : buffer_{ffx::make_device_buffer<Product>(queue)} {
      assert(reinterpret_cast<uintptr_t>(buffer_->data()) % alignof(Product) == 0);
    }
