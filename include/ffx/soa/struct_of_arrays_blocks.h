...
#define _ACCUMULATE_SOA_BLOCKS_SIZE_IMPL(VALUE_TYPE, NAME, LAYOUT_NAME)                      \
  _soa_impl_ret += LayoutFor<LAYOUT_NAME>::computeDataSize(                                  \
      ffx::soa::detail::extractSegment<LayoutFor<LAYOUT_NAME>, blocksNumber>(sizes, index)); \
  index += ffx::soa::detail::nBlocks<LayoutFor<LAYOUT_NAME>>();
...
#define _DECLARE_MEMBER_CONSTRUCTION_BLOCKS_IMPL(VALUE_TYPE, NAME, LAYOUT_NAME)                             \
  BOOST_PP_CAT(NAME, _) = LayoutFor<LAYOUT_NAME>(                                                           \
      mem + offset, ffx::soa::detail::extractSegment<LayoutFor<LAYOUT_NAME>, blocksNumber>(sizes_, index)); \
  offset += LayoutFor<LAYOUT_NAME>::computeDataSize(                                                        \
      ffx::soa::detail::extractSegment<LayoutFor<LAYOUT_NAME>, blocksNumber>(sizes_, index));               \
  index += ffx::soa::detail::nBlocks<LayoutFor<LAYOUT_NAME>>();
...
#define _COUNT_SOA_BLOCKS_IMPL(VALUE_TYPE, NAME, LAYOUT_NAME) \
  soa_blocks_count += ffx::soa::detail::nBlocks<LayoutFor<LAYOUT_NAME>>();
...
#define GENERATE_SOA_BLOCKS(CLASS, ...)                                                                                \
  ...
  struct CLASS {                                                                                                       \
    template <template <std::size_t, bool> class LAYOUT>                                                    \
    using LayoutFor = typename ffx::soa::LayoutParameters<ALIGNMENT>::template                                         \
    ...
    constexpr static bool alignmentEnforcement = ALIGNMENT_ENFORCEMENT;                                                \
                                                                                                                       \
    template <std::size_t VIEW_ALIGNMENT = ffx::soa::CacheLineSize::defaultSize,                            \
            bool VIEW_ALIGNMENT_ENFORCEMENT = ffx::soa::AlignmentEnforcement::relaxed,                                 \
    ...
    template <std::size_t VIEW_ALIGNMENT, bool VIEW_ALIGNMENT_ENFORCEMENT,                                             \
              bool TRestrictQualify, ffx::soa::range_checking::Mode TRangeChecking>                                     \
    struct ConstViewTemplateFreeParams {                                                                               \
      ...
      using ConstViewFor = typename LayoutT::template ConstViewTemplate<TRestrictQualify, TRangeChecking>;             \
                                                                                                                       \
      using size_type = ffx::soa::size_type;                                                                           \
      using byte_size_type = ffx::soa::byte_size_type;                                                                 \
    ...
    using ConstViewTemplate = ConstViewTemplateFreeParams<ALIGNMENT, ALIGNMENT_ENFORCEMENT, TRestrictQualify,          \
      TRangeChecking>;                                                                                                 \
                                                                                                                       \
    using ConstView = ConstViewTemplate<ffx::soa::restrict_qualify::default_value, ffx::soa::range_checking::default_value>;         \
                                                                                                                       \
    template <std::size_t VIEW_ALIGNMENT,                                                                   \
      ...
      : public ConstViewTemplateFreeParams<VIEW_ALIGNMENT, VIEW_ALIGNMENT_ENFORCEMENT,                                 \
                                           TRestrictQualify, TRangeChecking> {                                         \
      using BOOST_PP_CAT(CLASS, _parametrized) = CLASS<VIEW_ALIGNMENT, VIEW_ALIGNMENT_ENFORCEMENT>;                    \
      template <typename LayoutT>                                                                                      \
      using ViewFor = typename LayoutT::template ViewTemplate<TRestrictQualify, TRangeChecking>;                       \
      using base_type =                                                                                                \
        ConstViewTemplateFreeParams<VIEW_ALIGNMENT, VIEW_ALIGNMENT_ENFORCEMENT, TRestrictQualify, TRangeChecking>;     \
      using size_type = ffx::soa::size_type;                                                                           \
      using byte_size_type = ffx::soa::byte_size_type;                                                                 \
      using AlignmentEnforcement = ffx::soa::AlignmentEnforcement;                                                     \
                                                                                                                       \
