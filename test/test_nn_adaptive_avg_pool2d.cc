...
namespace ffx_runtime {
  ...
  extern "C" {
  extern const unsigned char _binary_test_nn_adaptive_avg_pool2d_bin_start[];
  extern const unsigned char _binary_test_nn_adaptive_avg_pool2d_bin_end[];
  }

  // ── parameter structs ─────────────────────────────────────────────────────

  template <std::size_t BatchSize, std::size_t InH, std::size_t InW, std::size_t Channels, std::size_t OutH, std::size_t OutW>
  struct adaptive_avg_pool2d_params_t {
    using kernel_t = ffx::nn::AdaptiveAvgPool2d<BatchSize, InH, InW, Channels, OutH, OutW>;
  ...
  using clean_halving_t = adaptive_avg_pool2d_params_t<1, 4, 4, 1, 2, 2>;
  using multi_channel_t = adaptive_avg_pool2d_params_t<1, 6, 4, 3, 2, 2>;
  using global_pool_t = adaptive_avg_pool2d_params_t<2, 8, 8, 4, 1, 1>;
  using non_divisible_t = adaptive_avg_pool2d_params_t<1, 7, 7, 2, 3, 3>;
  using asymmetric_output_t = adaptive_avg_pool2d_params_t<1, 8, 6, 1, 4, 3>;
  ...
  struct AdaptiveAvgPool2dTestCase {
    ...
    static const unsigned char* input() { return _binary_test_nn_adaptive_avg_pool2d_bin_start + input_offset; }
    static const unsigned char* output() { return _binary_test_nn_adaptive_avg_pool2d_bin_start + output_offset; }
  };

  // ── typed test boilerplate ────────────────────────────────────────────────

  template <typename T>
  class AdaptiveAvgPool2dTest : public ::testing::Test {};

  struct adaptive_avg_pool2d_test_name_t {
    template <typename T>
  ...
  TYPED_TEST_SUITE(AdaptiveAvgPool2dTest, impl_t, adaptive_avg_pool2d_test_name_t);

  // ── forward test ─────────────────────────────────────────────────────────

  TYPED_TEST(AdaptiveAvgPool2dTest, forward) {
    using Cfg = TypeParam;
    using Data = AdaptiveAvgPool2dTestCase<TypeParam>;
    ...
    using AdaptiveAvgPool2d = typename Cfg::kernel_t;

    alpaka::exec<Acc1D>(queue, grid, AdaptiveAvgPool2d{}, device_input.data(), device_output.data());

    alpaka::memcpy(queue, host_output, device_output);
    alpaka::wait(queue);

    for (std::size_t i = 0; i < output_size; ++i) {
      // 1e-4f tolerance is standard for average reductions due to floating point accumulation ordering
