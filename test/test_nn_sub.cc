...
namespace ffx_runtime {
  ...
  using sub_small_params_t = sub_params_t<16>;
  using sub_matrix_params_t = sub_params_t<8 * 32>;
  using sub_single_element_params_t = sub_params_t<1>;
  using sub_large_bulk_params_t = sub_params_t<1024>;
  using sub_non_divisible_params_t = sub_params_t<1007>;

  template <typename T>
  class SubTest : public ::testing::Test {};

  struct sub_test_name_t {
    template <typename T>
    static std::string GetName(const std::size_t index) {
      if constexpr (std::is_same_v<T, sub_small_params_t>)
        return "small";
      if constexpr (std::is_same_v<T, sub_matrix_params_t>)
        return "matrix";
      if constexpr (std::is_same_v<T, sub_single_element_params_t>)
        return "single_element";
      if constexpr (std::is_same_v<T, sub_large_bulk_params_t>)
        return "large_bulk";
      if constexpr (std::is_same_v<T, sub_non_divisible_params_t>)
        return "non_divisible";
      return std::to_string(index);
    }
  };

  using impl_t = ::testing::Types<sub_small_params_t,
                                  sub_matrix_params_t,
                                  sub_single_element_params_t,
                                  sub_large_bulk_params_t,
                                  sub_non_divisible_params_t>;

  TYPED_TEST_SUITE(SubTest, impl_t, sub_test_name_t);

  TYPED_TEST(SubTest, forward) {
    using Cfg = TypeParam;
    ...
    alpaka::exec<Acc1D>(
        queue, grid, ffx::nn::Sub<N>{}, device_input_a.data(), device_input_b.data(), device_output.data());
