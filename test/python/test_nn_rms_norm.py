...
TEST_CASES: List[TestCase] = [
    # Standard LLaMA-style sequence with affine scaling (gamma)
    TestCase("standard_affine", num_tokens=4, embedding_dim=64, has_gamma=True),
    # Edge case: single token
    TestCase("single_token", num_tokens=1, embedding_dim=128, has_gamma=True),
    # Pure RMSNorm — gamma=nullptr (scale defaults to 1.0)
    TestCase("no_affine", num_tokens=8, embedding_dim=32, has_gamma=False),
    # Large hidden dimension (typical LLM head size)
    TestCase("large_embedding", num_tokens=2, embedding_dim=512, has_gamma=True),
